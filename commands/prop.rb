...
class Command
  def self.prop(property = nil, name = nil)
    if name.nil?
      case property
      ...
      when 'ignore_updater'
        puts "Use the 'ignore_updater' property for packages that should not be updated with the GitHub automatic updater workflow."
      when 'is_fake'
      ...
      unless property.nil?
        @prop_pkg = Package.load_package(File.join(CREW_PACKAGES_PATH, "#{name}.rb"))
        puts @prop_pkg.send("#{property}?")
      end
    end
  end
end
