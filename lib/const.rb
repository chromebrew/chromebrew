...
if ENV['CI']
  ...
  CREW_RUST_VER = Kernel.system('command -v cargo', %i[out err] => File::NULL) ? "rust#{`cargo -V`.chomp.split[1].split('.')[0..1].join('.')}" : "rust#{crew_rust_ver_default}"
end
...
CREW_DOCOPT = <<~DOCOPT
  ...
  Usage:
    ...
    crew deps [options] [--deep] [--ordered] [-t|--tree] [-b|--include-build-deps] [--exclude-buildessential] [-v|--verbose] <name> ...
    crew diskstat [options] [-a|--all] [<count>]
    crew download [options] [-s|--source] [-v|--verbose] <name> ...
    crew files [options] [-f|--force] [-v|--verbose] <name> ...
