...
class Integration < Package
  ...
  def self.install
    ...
    Dir.chdir "chromebrew-supechicken-patch-1/integration" do
      FileUtils.install 'main.sh', "#{CREW_DEST_PREFIX}/bin/crew_integration", mode: 0755
      FileUtils.install 'x-terminal-emulator.sh', "#{CREW_DEST_PREFIX}/bin/x-terminal-emulator", mode: 0755
      FileUtils.install 'x-www-browser.sh', "#{CREW_DEST_PREFIX}/bin/x-www-browser", mode: 0755
