...
class Command
  def self.check(name, force)
    ...
    unless File.file?(local_package)
      ...
      puts 'No local package file found, skipping check.'.lightred if caller_locations(1, 2)[0].to_s.split[3].split('_')[0].to_s == 'check'
      return true
    end
    ...
    if to_copy && !force
      ...
      operation = caller_locations(1, 2)[0].to_s.split[3].split('_')[0]
      print "Would you like to copy #{name}.rb to crew and start the #{operation}? [Y/n] ".yellow
    ...
    system "#{CREW_LIB_PATH}/tests/prop_test #{name}"
    system "#{CREW_LIB_PATH}/tests/buildsystem_test #{name}"
  end
end
