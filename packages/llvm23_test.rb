...
class Llvm23_test < Package
  ...
  def self.patch
    ...
    patches = [
      # Fix for i686 needing -atomic. See https://github.com/llvm/llvm-project/issues/177701
  ...
  def self.build
    ...
    unless Dir.exist?('builddir')
      ...
      CLCPLUSPLUS_EOF
      # Stage 1 Build.
      puts 'Starting LLVM Stage 1 Build...'.lightpurple
    ...
    FileUtils.mkdir_p "#{CREW_DEST_DIR}/llvm_stage1"
    system "DESTDIR=#{CREW_DEST_DIR}/llvm_stage1 #{CREW_NINJA} -C builddir install"

    # Stage 2 build.
    puts 'Starting LLVM Stage 2 Build...'.lightpurple
    stage2_build_ld_library_path = "#{CREW_DEST_DIR}/llvm_stage1/#{CREW_LIB_PREFIX}:\$LD_LIBRARY_PATH"
    stage2_build_path = "#{CREW_DEST_DIR}/llvm_stage1/#{CREW_PREFIX}/bin:\$PATH"
    ...
    system "LD_LIBRARY_PATH=#{stage2_build_ld_library_path} PATH=#{stage2_build_path} \
    cmake -B builddir -G Ninja llvm \
          ...
          -DLLVM_TARGETS_TO_BUILD='#{@llvm_targets_to_build}' \
          -Wno-author"
    system "LD_LIBRARY_PATH=#{stage2_build_ld_library_path} PATH=#{stage2_build_path} #{CREW_NINJA} -C builddir -j #{CREW_NPROC}"
  end

  def self.install
    stage2_build_ld_library_path = "#{CREW_DEST_DIR}/llvm_stage1/#{CREW_LIB_PREFIX}:\$LD_LIBRARY_PATH"
    stage2_build_path = "#{CREW_DEST_DIR}/llvm_stage1/#{CREW_PREFIX}/bin:\$PATH"
    system "LD_LIBRARY_PATH=#{stage2_build_ld_library_path} PATH=#{stage2_build_path} DESTDIR=#{CREW_DEST_DIR} #{CREW_NINJA} -C builddir install"
