...
class Llvm_libs < Package
  ...
  def self.install
    puts 'Installing llvm to pull files for build...'.lightblue
    @filelist_path = File.join(CREW_META_PATH, 'llvm.filelist')
    abort 'File list for llvm does not exist!'.lightred unless File.file?(@filelist_path)
