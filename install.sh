#!/bin/bash
CREW_INSTALLER_VERSION=2026011501
# Exit on fail.
set -eE
...
/bin/mkdir -p "${CREW_PREFIX}/etc/env.d"
...
function curl_wrapper () {
  ...
  if [[ -f "/usr/bin/curl" ]] && LD_LIBRARY_PATH=/usr/lib${SYSTEM_LIB_SUFFIX}:/lib${SYSTEM_LIB_SUFFIX} /usr/bin/curl --help &>/dev/null; then
    ...
    DEBUG_OUT+="$(ldd ${CREW_PREFIX}/bin/curl 2> /dev/null)"
...
if [[ -n "${CREW_PRE_GLIBC_STANDALONE}" ]]; then
  ...
  BOOTSTRAP_PACKAGES+=' zlib zlib_ng libxcrypt upx patchelf lz4 xzutils gcc_lib crew_mvdir ca_certificates libyaml openssl findutils ncurses readline psmisc'
else
  ...
  [[ -n "${PREFIX_CMD}" ]] && BOOTSTRAP_PACKAGES+=' util_linux'
  BOOTSTRAP_PACKAGES+=' zlib zlib_ng libxcrypt upx patchelf lz4 xzutils crew_mvdir ncurses readline gcc_lib ca_certificates libyaml openssl gmp findutils psmisc'
...
if ! git --version; then
  ...
  DEBUG_OUT+=$(ldd "$(command -v git)" 2> /dev/null)
  echo_info_stderr "${DEBUG_OUT}"
