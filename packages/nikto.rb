...
class Nikto < Package
  ...
  depends_on 'perl_xml_writer'

  no_compile_needed

  def self.patch
    # Fix /usr/bin/perl: bad interpreter: No such file or directory
    system "sed -i 's,/usr/bin/perl,/usr/bin/env perl,' program/utils/replay.pl"
  end
  ...
  def self.install
    ...
    FileUtils.install 'documentation/nikto.1', "#{CREW_DEST_MAN_PREFIX}/man1/nikto.1", mode: 0o644
    ...
    FileUtils.ln_s "#{CREW_PREFIX}/share/nikto/utils/replay.pl", "#{CREW_DEST_PREFIX}/bin/replay"
    FileUtils.ln_s "#{CREW_PREFIX}/share/nikto/utils/nikto-bulk.sh", "#{CREW_DEST_PREFIX}/bin/nikto-bulk"
  end

  def self.postinstall
    ExitMessage.add <<~EOM

      To finish the installation, execute 'source #{HOME}/.bashrc'

      The configuration file is located at #{HOME}/nikto.conf.
