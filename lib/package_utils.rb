...
class PackageUtils
  ...
  def self.get_gem_vars(passed_name = nil, passed_version = '0', upstream_name = nil)
    ...
    if upstream_name.nil?
      passed_name.delete_prefix!('Ruby_')
      # We hardcode alternative package names here because the alternative, the fetcher.suggest_gems_from_name function, is very slow.
      if %w[
