...
class Pip < Package
  ...
  def self.install
    ...
    puts "trying to install #{@py_pkg}==#{@py_pkg_chromebrew_version}"
    Kernel.system "PYO3_USE_ABI3_FORWARD_COMPATIBILITY=1 MAKEFLAGS=-j#{CREW_NPROC} #{@pre_configure_options} python3 -s -m pip install --force-reinstall #{prerelease? ? '--pre' : ''} --ignore-installed -U #{@py_pkg}==#{@py_pkg_chromebrew_version}"
    get_pip_info(@py_pkg)
    ...
    if @py_pkg_chromebrew_version == @pip_pkg_version
      ...
      if get_pip_info(@py_pkg)
        ...
        Kernel.system "PYO3_USE_ABI3_FORWARD_COMPATIBILITY=1 python3 -m pip install #{prerelease? ? '--pre' : ''} -vvv --force-reinstall --upgrade '#{@py_pkg}==#{@py_pkg_chromebrew_version}'"
      end
    end
