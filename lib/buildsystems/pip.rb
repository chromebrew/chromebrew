  ...
  def self.install
    ...
    if @py_pkg_chromebrew_version == @pip_pkg_version
      ...
      @pip_wheel = if @source_url == 'SKIP'
                     ...
                     `PYO3_USE_ABI3_FORWARD_COMPATIBILITY=1 MAKEFLAGS=-j#{CREW_NPROC} #{@pre_configure_options} python3 -m pip wheel #{prerelease? ? '--pre' : ''} -w #{@pip_cache_dir} --platform-tag=#{PIP_ARCH} git+#{source_url}`[/(?<=filename=)(.*)*?(\S+)/, 0]
                   end
