...
echo "crosh and initiate a fresh installation of Chromebrew."
echo
if [ "${TERM}" != "xterm" ]; then
  echo "Unable to restore from crosh shell.  Enter a VT-2 session and try again."
...
case "${response}" in
  y|Y)
    ...
    ${SUDO} /bin/mkdir ${CREW_PREFIX}
    ${SUDO} /bin/chown ${CREW_USER}:${CREW_USER} ${CREW_PREFIX}
    ${SUDO} /bin/chmod 755 ${CREW_PREFIX}
    echo "Restore complete.  Press Ctrl + Alt + <- (left arrow) to exit the VT-2 Developer Console."
    ;;
  *)
