...
class Vala < Package
  ...
  depends_on 'graphviz' unless ARCH == 'i686'
  depends_on 'libxslt'
  depends_on 'glib'
  depends_on 'dbus'
  depends_on 'glibc' # R
  ...
  def self.build
    ...
    Dir.chdir('vala-bootstrap') do
      ...
      system 'VALAC=/no-valac mold -run ./configure --prefix=`pwd`/../bootstrap_install'
      system 'make'
      system 'make install'
    end

    system "VALAC=`pwd`/bootstrap_install/bin/valac mold -run ./autogen.sh \
      #{CREW_OPTIONS} \
