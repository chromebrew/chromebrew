...
class Opera < Package
  ...
  def self.install
    ...
    system "echo 'if [[ $(ls #{CREW_PREFIX}/bin/ | grep x-www-browser) = x-www-browser ]]; then echo x-www-browser is already set; else sudo ln -s #{CREW_PREFIX}/bin/opera #{CREW_PREFIX}/bin/x-www-browser; fi' > script.la.tmp"
