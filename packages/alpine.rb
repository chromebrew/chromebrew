...
class Alpine < Package                 # The first character of the class name must be upper case
  ...
  def self.build                   # the steps required to build the package
    system "./configure", "--prefix=#{CREW_PREFIX}", "--libdir=#{CREW_LIB_PREFIX}", "--with-ssl-dir=/etc/ssl", "--with-ssl-include-dir=/usr/local/include", "--with-ssl-lib-dir=#{CREW_LIB_PREFIX}", "--disable-nls", "--with-system-pinerc=#{CREW_PROFILE}/etc/alpine.d/pine.conf", "--with-sysytem-fixed-pinerc=#{CREW_PREFIX}/etc/alpine/pine.conf.fixed"
    system "make"
  end
